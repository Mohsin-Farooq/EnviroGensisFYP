...
namespace EnviroGenesis.WorldGen
{
    [ExecuteInEditMode]
    public class BiomeZone : MonoBehaviour
    {
                ...
                if (IsInsideZone(pos))
                {
                    ...
                    if (prefab != null) {
                        ...
                        if (!IsNearOther(pos, csize) && IsFitDensity(pos, density_dist, gsize)) {
                            ...
                            if (is_valid)
                            {
                                GameObject nobj = InstantiatePrefab(prefab, parent.transform, pos);
                                ...
                                Rigidbody rigid = nobj.GetComponent<Rigidbody>();
                                if (rigid != null)
                                {
                                    rigid.position = pos;
                                }
        ...
        private GameObject InstantiatePrefab(GameObject prefab, Transform parent, Vector3 pos)
        {
            GameObject nobj;
            ...
            nobj = Instantiate(prefab, pos, prefab.transform.rotation);
