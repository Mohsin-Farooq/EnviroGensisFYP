...
namespace EnviroGenesis
{
    ...
    public class InventoryData
    {
        ...
        public void TransferTo(InventoryData inventory, string item_id, int quantity)
        {
            for (int i = 0; i < size; i++)
            {
                InventoryItemData item = GetInventoryItem(i);
                if (item != null && item.item_id == item_id && item.quantity > 0 && quantity > 0)
                {
                    int q = Mathf.Min(item.quantity, quantity);
                    quantity -= q;
                    RemoveItemAt(i, q);

                    bool all = q == item.quantity;
                    string uid = all ? item.uid : UniqueID.GenerateUniqueID();
                    inventory.AddItem(item.item_id, q, item.durability, uid);
                }
            }
        }
